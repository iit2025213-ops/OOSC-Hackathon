...
const navItems = [
  ...
  { to: '/dashboard/schemes', icon: 'explore', labelKey: 'sidebar.scheme_navigator' },
  { to: '/dashboard/forms', icon: 'smart_toy', labelKey: 'sidebar.ai_form_filler' },
];
  ...
  return (
    <div className="min-h-screen overflow-x-hidden font-body-md text-body-md flex bg-surface text-on-surface selection:bg-primary selection:text-on-primary">
      ...
      <nav className="hidden md:flex bg-surface-container-low/20 backdrop-blur-2xl w-64 border-r border-white/5 shadow-sm fixed left-0 top-0 h-screen flex-col pt-24 pb-8 z-40">
        <div className="px-6 mb-12 notranslate">
          <h1 className="font-headline-lg text-headline-lg font-extrabold tracking-tight text-on-surface">{t('sidebar.title')}</h1>
          <p className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-widest mt-1">{t('sidebar.subtitle')}</p>
        </div>
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  <span className="group-hover:translate-x-1 transition-transform leading-tight break-words flex-1">{t(item.labelKey || `sidebar.${item.label.toLowerCase().replace(' ', '_')}`)}</span>
                </>
              )}
            </NavLink>
          ))}
        ...
        <div className="px-6 mt-auto">
          <button
            ...
            className="w-full bg-primary text-on-primary-fixed py-3 rounded-lg font-label-sm text-label-sm flex items-center justify-center gap-2 hover:bg-primary-fixed transition-colors shadow-[0_0_20px_rgba(255,180,161,0.2)] h-auto min-h-[48px] px-4"
          >
            <span className="material-symbols-outlined shrink-0">add</span>
            <span className="break-words leading-tight text-center">{t('sidebar.new_case')}</span>
          </button>
          <div className="mt-auto flex flex-col gap-2 pt-6 border-t border-white/5 pb-4">
            <NavLink
            ...
            >
              {({ isActive }) => (
                <>
                  <span className="material-symbols-outlined shrink-0" style={{ fontVariationSettings: isActive ? "'FILL' 1" : "'FILL' 0" }}>settings</span>
                  <span className="group-hover:translate-x-1 transition-transform leading-tight break-words flex-1">{t('sidebar.settings')}</span>
                </>
              )}
            </NavLink>
          </div>
      ...
      {/* TopNavBar (Mobile) */}
      <nav className="md:hidden bg-surface/10 backdrop-blur-xl border-b border-white/10 shadow-2xl fixed top-0 w-full z-50 flex justify-between items-center px-margin-mobile py-4 notranslate">
        <div className="font-headline-lg-mobile text-headline-lg-mobile font-bold text-on-surface">{t('sidebar.title')}</div>
          ...
          {navItems.slice(0, 3).map(item => (
            ...
            >
              {({ isActive }) => (
                <>
                  <span className="material-symbols-outlined shrink-0" style={{ fontVariationSettings: isActive ? "'FILL' 1" : "'FILL' 0" }}>{item.icon}</span>
                  <span className="font-label-sm text-[10px] uppercase tracking-wider text-center leading-tight break-words">{t(item.labelKey || `sidebar.${item.label.toLowerCase().replace(' ', '_')}`).split(' ').pop()}</span>
                </>
              )}
            </NavLink>
          ))}
